...
%% make sure to import project_workspace 
...
vreal = timetable2table(carPosition(:,4));
vreal = table2array(vreal(:,2));
vreal = vreal(6:17);
...
a = a(1:1140)-0.45;
%v = v(1:1140);
angle = -1*angle(11:1150)-46;
angle = smoothdata(angle);
phi = phi(1:1140) - phi(1);

tf = length(a)*dt;
t = linspace(0,tf,length(a));

tv = linspace(0,tf,length(vreal));

v = interp1(tv,vreal,t);
%v = smooth(v);
v = transpose(v);
